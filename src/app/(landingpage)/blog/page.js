...
const Page = () => {
  ...
  const handleReply = async (postId) => {
    ...
    updatedPost.answers = [...updatedPost.answers, newAnswer];
    await updateDoc(questionRef, { answers: updatedPost.answers });
    setNewAnswer("");
    fetchQuestions();
  };
    ...
    <div className="bg-white text-black w-full flex justify-center p-6">
      <div className="max-w-4xl w-full">
        ...
        <div className="mb-6">
          ...
          <input type="text" placeholder="Name" className="border p-2 w-full" value={newQuestion.name} onChange={(e) => setNewQuestion({ ...newQuestion, name: e.target.value })} />
          <input type="text" placeholder="Topic" className="border p-2 w-full mt-2" value={newQuestion.topic} onChange={(e) => setNewQuestion({ ...newQuestion, topic: e.target.value })} />
        ...
        <div className="space-y-6">
          {posts.map((post) => (
            <div key={post.id} className="border p-4 rounded-lg shadow-md relative">
              ...
              <p className="text-gray-600 text-sm mt-1">{post.date} - Asked by {post.name}</p>
    ...
    />
              ...
              <div className="absolute top-2 right-2">
                ...
                {menuOpen === post.id && (
                  <div className="absolute right-0 mt-2 w-32 bg-white border rounded shadow-lg">
                    ...
                    <button onClick={() => handleDelete(post.id)} className="block w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-gray-100">Delete</button>
                  </div>
                )}
              </div>
              ...
              {openPost === post.id && (
                <div className="mt-2 text-gray-700 border-t pt-2">
                  ...
                  <textarea placeholder="Your Answer" className="border p-2 w-full mt-2" value={newAnswer} onChange={(e) => setNewAnswer(e.target.value)} />
                  <button onClick={() => handleReply(post.id)} className="bg-green-500 text-white p-2 rounded mt-2">Reply</button>
                </div>
              )}
            </div>
          ))}
